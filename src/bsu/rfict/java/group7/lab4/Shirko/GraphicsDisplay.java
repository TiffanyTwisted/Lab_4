...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[] {5,1,1,1,1,1,2,1,2}, 0.0f);
...
прямой соединяется со
     ...
     */
        ...
        for (int i = 0; i < graphicsData.length; i++) {
            ...
            Point2D.Double point = xyToPoint(graphicsData[i][0]+0.1,
                    graphicsData[i][1]+0.1);
            if (i > 0) {
...
его центра
       и угла прямоугольника, в который он вписан
        ...
        for (Double[] point : graphicsData) {
            ...
            arrow.lineTo(arrow.getCurrentPoint().getX()+5, arrow.getCurrentPoint().getY());
            arrow.lineTo(arrow.getCurrentPoint().getX()+5, arrow.getCurrentPoint().getY() -3);
            arrow.lineTo(arrow.getCurrentPoint().getX() -5, arrow.getCurrentPoint().getY()-3);
            arrow.lineTo(arrow.getCurrentPoint().getX()-5, arrow.getCurrentPoint().getY() );
            arrow.lineTo(arrow.getCurrentPoint().getX()+5, arrow.getCurrentPoint().getY() );
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if (minY <= 5.0 && maxY >= -5.0) {
            ...
            // а нижняя (minY) <= 0.0
            canvas.draw(new Line2D.Double(xyToPoint(minX, minY+0.1),
                    xyToPoint(maxX, minY+0.1)));
            // Стрелка оси X
            GeneralPath arrow = new GeneralPath();
